...
import { each, trimZero, addZero, getMaxDay, parseRow, parseDate, getElement, toElement, removeElement, noonCheck } from './util'
  ...
  _makeData (type, year, month) {
    ...
    if (type === 'hour' && this.config.hourList) {
      data = this.config.hourList.map(hour => {
        return {
          name: this.config.checkNoon ? noonCheck(hour) : parseRow(config['hourRow'], hour),
          value: addZero(hour)
        }
      })
    }
